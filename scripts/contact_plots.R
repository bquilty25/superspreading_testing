...
line_plot <- contact_data %>% 
  ...
  scale_y_continuous("Proportion of participants (%)"#,expand = expansion(mult = c(0,0.05))
                     )+
...
line_plot/dot_plot/nbinom_plot+plot_annotation(tag_levels = "A")+plot_layout(heights=c(1.5,1,1))
ggsave("results/contacts_.png",width=210,height=300,dpi=600,units="mm",bg="white")
