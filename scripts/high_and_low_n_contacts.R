...
contact_data %>% 
  ...
  select.(period,date_start, date_end, n, over_5, over_10, over_20, over_50, over_100, over_200) %>% 
  mutate.(across.(c(over_5:over_200),function(x)formatC(x*100,digits = 1,format="f")),
          date_start=format(date_start,"%d %b %Y"),
          date_end=format(date_end,"%d %b %Y")) %>% 
  htmlTable::htmlTable(rnames = F,header = c("Time period","From", "To", "N", "Over 5", "Over 10", "Over 20", "Over 50", "Over 100", "Over 200"))
